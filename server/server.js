...
app.post('/login',(req,res)=>{
  ...
  const query=`select user_id,password from users where username='${username}'`;
  connection.query(query,(error,results)=>{
    if(error){
      ...
      //console.log('OK') #drycoding
         ...
         if(results.length>0){
          const resuser_id= results[0].user_id;
          const IsvalidPassword=bcrypt.compareSync(password,results[0].password);
          if(IsvalidPassword){
          
           
           
            res.status(200).json({message:"login successful",user_id:resuser_id});
